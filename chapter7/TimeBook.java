...
public class TimeBook {
	...
	private int[] weekHours; //weekHours[i] has the week's hours worked for employee i + 1.
	private int[] dayHours; //dayHours[i] has the total hours worked by all employees on day i.
	...
	*/
	...
	private void computeWeekHours(){
		for(int employeeNumber = 1; employeeNumber <= numberOfEmployees; employeeNumber++){ //Process one employee.
			int sum = 0;
			
			for(int day = MON; day <= FRI; day++){
				sum = sum + hours[day][employeeNumber - 1];
				//Sum contains the sum of all the hours worked in one
				//week by the employee with employeeNumber.
			}
	...
	private void computeDayHours(){
		for(int day = MON; day <= FRI; day++){ //Process one day (for all employees).
			int sum = 0;
	...
	//Convert 0 to "Monday", 1 to "Tuesday", etc.
	//Blanks are inserted to make all string the same length.
